...
import { entradaDeposito,EntradaDeposito, buscarDeposito, buscarDepositoId } from "../service/estoque";
...
export async function buscar(req: Request, res: Response) {
    const result = await buscarDeposito()
...
export async function buscarPorId(req: Request, res: Response) {
    const { id } = req.params
    const result = await buscarDepositoId(id)
